...
- **パス**: 未定（新規リポジトリ。my_crm と同系統スタックで別リポジトリ想定）
...
- 配色案: 自然の深緑を基調＋AIを示す一点のアクセント（金/琥珀 or 落ち着いた青）。静かで疲れぬトーン。
...
> 配色・トーンは上記ブランド方針（深緑基調＋AIアクセント、静かで疲れぬ）に準拠。
...
- [ ] UI確定後、構想書を更新し、家老へ実装下知（cmd）を発する。
